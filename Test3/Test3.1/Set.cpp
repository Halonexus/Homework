...
void printTreeAscendingNoCopies(const TreeElement* const & element)
{
	if (element)
	{
		printTreeAscendingNoCopies(element->leftChild);
		printf("%d is found %d times\n", element->value, element->amount);
		printTreeAscendingNoCopies(element->rightChild);
	}
	return;
}
...
{
	printTreeAscendingNoCopies(set.tree.root);
	return;
}
