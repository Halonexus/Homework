...
void printResults(HashTable* table)
{
	...
	for (int i = 0; i < table->size; i++)
	{
		if (!isEmpty(table->table[i]))
		{
			...
			fputs(" ", stdout);
			String temp;
			temp.cString = intToString(table->uses[i]);
	...
	String temp;
	temp.cString = intToString(getMaxTries());
	fputs("Max amount of tries: ", stdout);
	fputs(temp.cString, stdout);
	fputs("\n", stdout);
	deleteString(temp);
	temp = getMaxTryString();
	fputs("The word with most tries: ", stdout);
	fputs(temp.cString, stdout);
	fputs("\n", stdout);
	deleteString(temp);
	temp.cString = intToString(getTotalWords());
	fputs("Total amount of words: ", stdout);
	fputs(temp.cString, stdout);
	fputs("\n", stdout);
	deleteString(temp);
	temp.cString = intToString(getUnused(table));
